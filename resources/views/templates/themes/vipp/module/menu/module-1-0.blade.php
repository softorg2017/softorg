                    ...
                    @foreach($items as $v)
                        ...
                        <div class="media" role="button">
                            ...
                            <div class="media-body">
                                <div class="media-row media-title-row multi-ellipsis-2">
                                    <a href="{{url(config('common.org.front.prefix').'/item/'.encode($v->id))}}">
                                        <b> {{ $v->title or '' }} </b>
                                    </a>
                                </div>
                                <div class="media-row media-description-row multi-ellipsis-1">{{ $v->description or '' }}</div>
                                <div class="media-row media-time-row text-grey">{{ $v->updated_at->format('Y-m-d') }}</div>
                            </div>
                        </div>
