        ...
        <div class="row full block-in">
            ...
            <div class="module-block-container" style="display:none;">
                <div class="row full block-{{$data->column}}-column">
                    ...
                    @foreach($data->menu->items as $v)
                        <li class="item-block item-block-1-1 wicked-barrelRoll-hover" role="button">
                            <a href="{{url(config('common.org.front.prefix').'/item/'.encode($v->id))}}">

                                <div class="item-block-row item-block-top item-block-1-1-{{$loop->parent->index}}-{{$loop->index}}">
                                    @if(!empty($v->cover_pic))
                                        <img src="{{ config('common.host.'.env('APP_ENV').'.cdn').'/'.$v->cover_pic }}" alt="">
                                    @else
                                        {{--<script>--}}
                                            {{--initialise(".item-block-1-1-{{$loop->parent->index}}-{{$loop->index}}");--}}
                                        {{--</script>--}}
                                    @endif
                                </div>
