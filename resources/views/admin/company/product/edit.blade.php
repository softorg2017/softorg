                ...
                <div class="form-group">
                    ...
                    <div class="col-md-8 ">
                        <select class="form-control" onchange="select_menu()">
                            ...
                            @if(!empty($data->company->menus))
                            @foreach($data->company->menus as $v)
                            <option data-id="{{$v->id}}" @if($data->menu_id == $v->id) selected="selected" @endif>{{$v->name}}</option>
                            @endforeach
                            @else
                            @foreach($company->menus as $v)
...
<script>
    ...
    $(function() {
        ...
        $("#edit-product-submit").on('click', function() {
            var options = {
                url: "/admin/company/product/edit",
                type: "post",
                dataType: "json",
                // target: "#div2",
                success: function (data) {
                    ...
                    {
                        layer.msg(data.msg);
                        location.href = "/admin/company/product/list";
                    }
                }
            };
