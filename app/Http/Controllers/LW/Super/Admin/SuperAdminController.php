<?php
namespace App\Http\Controllers\LW\Super\Admin;
...
use App\Repositories\LW\Super\Admin\SuperAdminRepository;
    ...
    public function operate_user_user_login()
    {
        ...
        if($user)
        {
            ...
            if($type == "gps")
            {
                ...
                {
                    $url = env('LW_DOMAIN_GPS').'/admin';
                    if (stripos($url, "http://") === false && stripos($url, "https://") === false)
                    {
                        $url = env('DOMAIN_PROTOCOL') . $url;
                    }
                    return redirect($url);
                }
            ...
            else
            {
                ...
                if($this->env == 'test')
                {
                    ...
                    if(request()->isMethod('get'))
                    {
                        return redirect(env('DOMAIN_TEST_DEFAULT'));
                    }
...
//                else if(request()->isMethod('post')) return response_success($return);
    ...
    public function view_user_list()
    {
        ...
        else if(request()->isMethod('post')) return $this->repo->get_datatable_by_user_list(request()->all());
    }
    ...
    public function view_item_list()
    {
        ...
        else if(request()->isMethod('post')) return $this->repo->get_datatable_by_item_list(request()->all());
    }
    // 【内容】【原子】返回-列表-视图
    public function view_item_list_for_atom()
    {
        if(request()->isMethod('get')) return $this->repo->view_item_list_for_atom(request()->all());
        else if(request()->isMethod('post')) return $this->repo->get_datatable_by_item_list_for_atom(request()->all());
    }
