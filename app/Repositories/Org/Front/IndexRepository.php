<?php
namespace App\Repositories\Org\Front;
...
//            }
            ...
            return view(config('common.org.view.frontend.online').'.entrance.index')
                ->with(['org'=>$org,'menus'=>$org->menus]);
        }
        else dd("企业不存在");
    }
    // 返回（前台）【主页】视图
...
//        $decode_id = decode($encode_id);
        ...
        if($item)
        {
            ...
            $record["module"] = 0; // module=0 item
            $record["org_id"] = $item->org->id;
            $record["page_id"] = $decode_id;
            $record["from"] = request('from',NULL);
            $this->record($record);
    ...
    // 返回（前台）【目录】列表页视图
    public function view_menu1($org)
    {
...
//                }
    ...
    public function share($post_data)
    {
        ...
        if(!in_array($sort, [1,2,3])) return response_error([],"参数sort错误");
        ...
        $org = OrgOrganization::where('website_name',$website_name)->first();
        if($org)
        {
            $record["org_id"] = $org->id;
            $page_encode = $post_data["page_id"];
            $page_decode = decode($page_encode);
            if(intval($page_decode) !== 0 && !$page_decode) return response_error();
            $record["page_id"] = $page_decode;
            ...
            {
                $menu = OrgMenu::find($page_decode);
                if($menu)
                {
                    if($menu->org_id != $org->id) return response_error([],"参数有误");
                    // 访问数量+1
                    $menu->timestamps = false;
                    $menu->increment('share_num');
                }
                else return response_error([],"目录不存在");
            }
            // detail 内容item分享
            else if($sort == 3)
            {
                $item = OrgItem::find($page_decode);
                if($item)
                {
                    if($item->org_id != $org->id) return response_error([],"参数有误");
                    // 访问数量+1
                    $item->timestamps = false;
                    $item->increment('share_num');
                }
                else return response_error([],"内容不存在");
            }
        }
