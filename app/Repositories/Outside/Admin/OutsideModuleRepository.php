...
class OutsideModuleRepository {

    private $model;
    private $repo;
    public function __construct()
    {
        $this->model = new OutsideModule;
    }
    ...
    public function get_list_datatable($post_data)
    {
        ...
        $query = OutsideModule::select("*")->with(['admin', 'menu', 'menus']);
        $total = $query->count();
    ...
    public function view_create()
    {
        ...
        return view('outside.admin.module.edit')->with(['operate'=>'create', 'encode_id'=>encode(0), 'menus'=>$menus]);
    }
    ...
    public function view_edit()
    {
        ...
        $menus = OutsideMenu::get();
        if($decode_id == 0) return view('outside.admin.module.edit')->with(['operate'=>'create', 'encode_id'=>$id]);
        else
        {
            $module = OutsideModule::with(['menu','menus'])->find($decode_id);
            if($module)
            {
                unset($module->id);
                return view('outside.admin.module.edit')->with(['operate'=>'edit', 'encode_id'=>$id, 'data'=>$module, 'menus'=>$menus]);
            }
    ...
    public function save($post_data)
    {
        ...
        {
            $module = new OutsideModule;
            $post_data["admin_id"] = $admin->id;
        }
        else if($operate == 'edit') // 修改
        {
            $module = OutsideModule::find($decode_id);
...
//                    $module->menus()->attach($post_data["menus"]);
                ...
                if($post_data["type"] == 4 && !empty($post_data["multiples"]))
                {
                    foreach($post_data["multiples"] as $num => $m) {
                        ...
                        if(!empty($m["file"]))
                        {
                            ...
                            $result = $upload->create($m["file"], 'outside-common-img_multiple', rand(100,999).$num);
                            if($result["status"])
                            {
                ...
                if($post_data["type"] == 5 && !empty($post_data["carousels"]))
                {
                    foreach($post_data["carousels"] as $num => $c) {
                        ...
                        {
                            $upload = new CommonRepository();
                            $result = $upload->create($c["file"], 'outside-common-img_multiple', rand(100,999).$num);
                            if($result["status"])
                            {
                ...
                {
                    $upload = new CommonRepository();
                    $result = $upload->upload($post_data["cover"], 'outside-unique-modules' , 'cover_module_'.$encode_id);
                    if($result["status"])
                    {
...
//            exit($e->getMessage());
    ...
    public function view_sort()
    {
        ...
        $module = OutsideModule::orderBy('order', 'asc')->get();
        return view('outside.admin.module.sort')->with(['data'=>$module]);
    }
