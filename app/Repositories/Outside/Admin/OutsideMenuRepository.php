...
class OutsideMenuRepository {

    private $model;
    private $repo;
    public function __construct()
    {
        $this->model = new OutsideMenu;
    }
    ...
    public function get_list_datatable($post_data)
    {
        ...
        $query = OutsideMenu::select("*")->withCount(['items'])->with(['admin']);
        $total = $query->count();
    ...
    public function get_items_list_datatable($post_data)
    {
        ...
        $org_id = Auth::guard("outside_admin")->user()->org_id;
//        $query = OrgItem::select("*")->with(['admin','menus'])->where('id',$decode_id);
        $query = OutsideMenu::find($decode_id)->items()->with(['admin','menus']);
        $total = $query->count();
    ...
    public function view_items()
    {
        ...
        $menu = OutsideMenu::find($decode_id);
        if($menu)
        {
            unset($menu->id);
    ...
    {
        return view('outside.admin.menu.edit')->with(['operate'=>'create', 'encode_id'=>encode(0)]);
    }
    ...
    public function view_edit()
    {
        ...
        if($decode_id == 0) return view('outside.admin.menu.edit')->with(['operate'=>'create', 'encode_id'=>$id]);
        else
        {
            $menu = OutsideMenu::with(['items'])->find($decode_id);
            if($menu)
            {
                unset($menu->id);
                return view('outside.admin.menu.edit')->with(['operate'=>'edit', 'encode_id'=>$id, 'data'=>$menu]);
            }
            else return response("该目录不存在！", 404);
        }
    }
        ...
        try
        {
            ...
            if($bool) {
                ...
                if(!empty($post_data["cover"]))
                {
                    ...
                    $result = $upload->upload($post_data["cover"], 'outside-unique-menus' , 'cover_menu_'.$encode_id);
                    if($result["status"])
                    {
...
//            exit($e->getMessage());
    ...
    public function view_sort()
    {
        ...
        $menu = OutsideMenu::where(['org_id'=>$org_id])->orderBy('order', 'asc')->get();
        return view('outside.admin.menu.sort')->with(['data'=>$menu]);
    }
