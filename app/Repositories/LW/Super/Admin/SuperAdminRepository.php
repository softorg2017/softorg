<?php
namespace App\Repositories\LW\Super\Admin;
...
//        view()->share('auth_check',$this->auth_check);
    ...
    public function view_admin_index()
    {
        ...
        return view(env('LW_TEMPLATE_SUPER_ADMIN').'index')->with(['data'=>$me]);
    }
    ...
    {
        $me = Auth::guard('admin')->user();
        return view(env('LW_TEMPLATE_SUPER_ADMIN').'entrance.info.index')->with(['data'=>$me]);
    }
    // 【基本信息】返回-编辑-视图
    public function view_info_edit()
    {
        $me = Auth::guard('admin')->user();
        return view(env('LW_TEMPLATE_SUPER_ADMIN').'entrance.info.edit')->with(['data'=>$me]);
    }
    // 【基本信息】保存数据
...
//            exit($e->getMessage());
    ...
    {
        $me = Auth::guard('admin')->user();
        return view(env('LW_TEMPLATE_SUPER_ADMIN').'entrance.info.password-reset')->with(['data'=>$me]);
    }
    // 【密码】保存数据
...
//        if(!is_numeric($type)) return view(env('LW_TEMPLATE_SUPER_ADMIN').'errors.404');
//        if(!in_array($type,[1,2,3,10,11,88])) return view(env('LW_TEMPLATE_SUPER_ADMIN').'errors.404');
    ...
    public function view_user_user_create()
    {
        ...
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.user.user-edit';
    ...
    public function view_user_user_edit()
    {
        ...
        else
        {
            ...
            if($mine)
            {
                if(!in_array($mine->user_category,[1,9,11,88])) return view(env('LW_TEMPLATE_SUPER_ADMIN').'errors.404');
            ...
            else return view(env('LW_TEMPLATE_SUPER_ADMIN').'errors.404');
        }
    }
    // 【用户】【组织】保存数据
...
//            exit($e->getMessage());
    ...
    {
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.user.user-list';
        $view_return['menu_active_of_user_list'] = 'active menu-open';
        return view($view_blade)->with($view_return);
    }
    // 【用户】返回-列表-数据
    public function get_datatable_by_user_list($post_data)
    {
        $query = User::select('*')
            ->with([
                'district',
                'creator'=>function($query) { $query->select('id','username','nickname'); }
            ])
...
//            ->whereIn('usergroup',['Agent','Agent2']);
        ...
        {
            if(!in_array($post_data['category'],[0,-1]))
            {
                $query->where('user_category', $post_data['category']);
            }
        }
...
//        dd($list->toArray());
    ...
    {
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.item.item-list';
        $view_return['menu_active_of_item_list'] = 'active menu-open';
        return view($view_blade)->with($view_return);
    }
    // 【内容】【全部】返回-列表-数据
    public function get_datatable_by_item_list($post_data)
    {
        $me = Auth::guard('super')->user();
        $query = Def_Item::select('*')->withTrashed()
            ->with('owner','creator')
        ...
        if(isset($post_data['category']))
        {
            if(!in_array($post_data['category'],[0,-1]))
            {
                $query->where('item_category', $post_data['category']);
            }
        }
...
//        dd($list->toArray());
    ...
    {
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.item.item-list-for-atom';
        $view_return['menu_active_of_item_list_for_atom'] = 'active menu-open';
        return view($view_blade)->with($view_return);
    }
    // 【内容】【全部】返回-列表-数据
    public function get_datatable_by_item_list_for_atom($post_data)
    {
        $me = Auth::guard('super')->user();
...
//        dd($list->toArray());
    ...
    public function view_district_create()
    {
        ...
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.district.district-edit';
        return view($view_blade)->with([
            'operate'=>'create',
            'operate_id'=>0,
            'category'=>'item',
    ...
    public function view_district_edit()
    {
        ...
        else
        {
            ...
            {
//                if(!is_numeric($type)) return view(env('LW_TEMPLATE_SUPER_ADMIN').'errors.404');
                if(!in_array($mine->district_type,[1,2,3,4,11,21,31,41])) return view(env('LW_TEMPLATE_SUPER_ADMIN').'errors.404');
                $mine->custom = json_decode($mine->custom);
                $mine->custom2 = json_decode($mine->custom2);
                $mine->custom3 = json_decode($mine->custom3);
...
//            exit($e->getMessage());
    ...
    {
        return view(env('LW_TEMPLATE_SUPER_ADMIN').'entrance.district.district-list-for-all')
            ->with([
...
//            else $shared_root_scale[$k]->name = "其他";
        ...
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.statistic.statistic-index';
    ...
    public function view_statistic_user($post_data)
    {
        ...
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.statistic.statistic-user';
        return view($view_blade)->with($view_data);
    }
    // 【】流量统计
    public function view_statistic_item($post_data)
    {
        ...
        $view_blade = env('LW_TEMPLATE_SUPER_ADMIN').'entrance.statistic.statistic-item';
        return view($view_blade)->with($view_data);
    }
    ...
    {
        return view(env('LW_TEMPLATE_SUPER_ADMIN').'entrance.statistic.statistic-all-list')
            ->with([
